...
def check_jaxpr_constants(closed_jaxpr: core.ClosedJaxpr):
  ...
  if not (num_report := config.captured_constants_report_frames.value):
    message += (
  ...
  message += (
      ...
      f"Largest {min(num_report, len(closed_jaxpr.consts))} allocation(s):\n"
    )

  nbytes_var_const = zip(nbytes_iter, closed_jaxpr.jaxpr.constvars, closed_jaxpr.consts)
  for (nbytes, var, const) in heapq.nlargest(num_report, nbytes_var_const, key = operator.itemgetter(0)):
    ...
    for eqn in jaxpr_util.eqns_using_var(closed_jaxpr.jaxpr, var):
      call_frame_source_info = source_info_util.summarize(eqn.source_info, num_report)
...
def lower_jaxpr_to_fun(
  ...
  jaxpr_util.check_jaxpr_constants(jaxpr)
